...
async function setupWebpack() {
  ...
  if (!fs.existsSync(themesDir) || !fs.readdirSync(themesDir).length) {
    console.error(
      "Error: No themes directory found in /wp-content/themes or it is empty."
    );
    process.exit(1);
  }
  ...
  const themeDirs = fs
    ...
    .filter((dir) => fs.lstatSync(path.join(themesDir, dir)).isDirectory());
  ...
  console.log(
    "✔ Webpack config written to the root directory: webpack.config.js"
  );
  ...
  console.log(
    "✔ Setup complete. Webpack config and dependencies are ready for use."
  );
}
